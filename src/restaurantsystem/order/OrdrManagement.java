...
public class OrdrManagement {
	...
	private void openFile() {
		try {
			scan = new Scanner(new File(classLoader.getResource("./data/item.txt").getFile()));
	...
	public void ModifyItem() {
		...
		try {
			Scanner sc = new Scanner(new FileInputStream(classLoader.getResource("./data/item.txt").getFile()));
		...
		try {
			PrintWriter pw = new PrintWriter(new FileOutputStream(classLoader.getResource("./data/item.txt").getFile()));
			for (int i = 0; i < c; i++) {
				if (i == temp + 2) {
	...
	public void temp(int n, int i) {
		...
		try {
			PrintWriter pw = new PrintWriter(new FileOutputStream(classLoader.getResource("./data/temp.txt").getFile(), true));
	...
	public void delete() {
		...
		String quantity = "";
		try {
			Scanner sc = new Scanner(new FileInputStream(classLoader.getResource("./data/temp.txt").getFile()));
			while (sc.hasNextLine()) {
				name = sc.nextLine();
